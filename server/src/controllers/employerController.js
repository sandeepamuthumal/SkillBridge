...
export async function createEmployer(req, res) {
    try {
        const {userId, companyName, companyDescription,
                companyWebsite, logoUrl, industry,
                companySize, foundedYear, contactPersonName,
                verified, verificationDocuments, contactInfo,
                socialLinks} = req.body;

        const createdEmployer = new Employer({userId:userId,
            companyName:companyName,companyDescription:companyDescription, companyWebsite:companyWebsite,
            logoUrl:logoUrl,industry:industry,companySize:companySize,
            foundedYear:foundedYear, contactPersonName:contactPersonName,
            verified:verified, verificationDocuments:verificationDocuments,
            contactInfo:contactInfo, socialLinks:socialLinks
        })

        await createdEmployer.save();
        res.status(201).json({
            companyName: companyName, logoUrl:logoUrl, 
...
export async function getEmployerById(req,res) {
    try {
        ...
        res.status(500).json({message: "Error Occured When Get Specific Employer",
            employer: id});
...
export async function deleteEmployerById(req,res) {
    try {
        ...
        res.status(500).json({message: "Error Occured When Deleting Employer",
            employer: id});
