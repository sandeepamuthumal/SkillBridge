    ...
    try {
        ...
        if (statusFilterParam) {
            if (statusFilterParam === 'pending') {
                ...
                query.status = { $in: ['Draft', 'Paused', "Published", "Closed" , "Expired"] };
                console.log('SERVER: Filter applied: Pending Approval (isApproved:false, status:{ $in: [\'Draft\', \'Paused\', \'Published\', \'Closed\', \'Expired\'] })');
