                            ...
                            <div className="flex items-start space-x-6 mb-6">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex items-center space-x-1 mb-3">
                                        ...
                                        <span className="text-xl font-semibold text-gray-900">{job.employerId.companyName}</span>
                                        <span className="text-gray-500">•</span>
                                        <span className="text-gray-600">{job.employerId.companySize} employees</span>
                                    </div>

                                    <div className="flex flex-wrap items-center gap-4 text-gray-600 mb-4">
                                        <div className="flex items-center space-x-1">
                                            <MapPin className="w-4 h-4" />
                                            <span>{job.cityId.name}, {job.cityId.country}</span>
                                        </div>
                                        <div className="flex items-center space-x-1">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                                    ...
                                    <p className="text-lg font-bold text-green-900">
                                        {formatSalary(job.salaryRange.min, job.salaryRange.max, job.salaryRange.currency, job.salaryRange.period)}
                                    </p>
                                ...
                                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                                    ...
                                    <p className="text-lg font-bold text-blue-900">{formatDate(job.deadline)}</p>
                                    <p className={`text-sm ${daysLeft <= 3 ? 'text-red-600' : daysLeft <= 7 ? 'text-orange-600' : 'text-blue-600'
                                        }`}>
                                        {isExpired ? 'Applications closed' : `${daysLeft} days remaining`}
                                    </p>
                                </div>
                                ...
                                <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
                                    ...
                                    <p className="text-lg font-bold text-purple-900">{job.experienceLevel}</p>
                            ...
                            {!isExpired && (
                                <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
                                    <p className="text-sm text-orange-800">
                                        <strong>{job.maxApplications - job.applicationCount}</strong> spots remaining out of <strong>{job.maxApplications}</strong> total positions
                                    </p>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <section className="bg-white rounded-2xl p-8 shadow-sm border border-gray-200">
                            ...
                            <div className="flex flex-wrap gap-3">
                                {job.preferredSkills.map((skill, index) => (
                                    <span key={index} className="bg-blue-100 text-blue-800 px-4 py-2 rounded-xl font-medium border border-blue-200">
                                        {skill}
                                    </span>
                                ))}
                            </div>
                        ...
                        {job.applicationProcess && (
                            <section className="bg-blue-50 rounded-2xl p-8 border border-blue-200">
                                <h2 className="text-2xl font-bold text-blue-900 mb-4">Application Process</h2>
                                <p className="text-blue-800 leading-relaxed">{job.applicationProcess}</p>
                            </section>
                        )}
                    </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-semibold text-gray-900">{job.categoryId.name}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-gray-600">Employment Type</span>
                                    <span className="font-semibold text-gray-900">{job.typeId.name}</span>
                                </div>
                                <div className="flex justify-between">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-semibold text-gray-900">{job.workArrangement}</span>
                                </div>
                                {job.startDate && (
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">Start Date</span>
                                        <span className="font-semibold text-gray-900">{formatDate(job.startDate)}</span>
                                    </div>
                                )}
                                {job.duration && (
                                    <div className="flex justify-between">
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
                            ...
                            <div className="flex items-center space-x-4 mb-4">
                                ...
                                <div>
                                    <h4 className="font-bold text-lg text-gray-900">{job.employerId.companyName}</h4>
                                    <p className="text-gray-600">{job.employerId.industry}</p>
                                </div>
                            </div>

                            {job.employerId.companyDescription && (
                                <p className="text-gray-700 mb-4 leading-relaxed">{job.employerId.companyDescription}</p>
                            )}

                            <div className="space-y-2 mb-4">
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-600">Company Size</span>
                                    <span className="font-medium">{job.employerId.companySize} </span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-600">Founded</span>
                                    <span className="font-medium">{job.employerId.foundedYear}</span>
                                </div>
                            </div>
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {job.tags.map((tag, index) => (
                                    <span key={index} className="bg-gray-100 text-gray-700 px-3 py-2 rounded-lg text-sm font-medium border border-gray-200">
                                        {tag}
                                    </span>
                                ))}
                            </div>
                        ...
                        {job.contactEmail && (
                            <div className="bg-green-50 border border-green-200 rounded-2xl p-6">
                                <h3 className="text-lg font-bold text-green-900 mb-2">Questions?</h3>
